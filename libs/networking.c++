...
bool networking::network_structures::host::create_connection_address() {
    ...
    if (not this->connect_address_) {
        ...
        if (this->host_.empty()) {
            line_ = __LINE__ - 1;
            message = "Cannot retrieve address information without a hostname specified";
        ...
        if (this->port_.empty()) {
            line_ = __LINE__ - 1;
            message = "Cannot retrieve address information without a port";
            if (this->throw_except_) {
                throw exceptions::getaddrinfo_failure(message, true, __FILE__, line_, __FUNCTION__);
            }
            std::cerr << message << std::endl;
        ...
        if (status) {
            line_ = __LINE__ - 1;
            message = "Failed to retrieve address information for host \"" + this->host_ + "\". Error " + std::string(get_socket_error_string(status));
            if (this->throw_except_) {
                throw exceptions::getaddrinfo_failure(message, true, __FILE__, line_, __FUNCTION__);
            }
            std::cerr << message << std::endl;
...
bool networking::network_structures::host::create_connection_socket() {
    ...
    if (not valid_socket(this->connect_socket_)) {
        ...
        if (not valid_socket(this->connect_socket_)) {
            if (this->throw_except_) {
                throw exceptions::create_socket_failure(message + ". Error : " + std::string(get_socket_error_string(socket_error)), line_, __FUNCTION__);
            }
            std::cerr << message << std::endl;
            return false;
        }
...
bool networking::network_structures::host::main_socket_connected() const {
    return networking::socket_is_connected(this->connect_socket_, this->throw_except_);
}
...
networking::network_structures::host::host(const networking::network_structures::host& other) {
    ...
    this->throw_except_ = other.throw_except_;
...
networking::network_structures::host& networking::network_structures::host::operator=(networking::network_structures::host&& other) noexcept {
    if (this != &other) {
        ...
        other.connect_socket_ = invalid_socket;
        other.host_ = other.port_ = "";
        other.tcp_ = other.serving_ = other.print_except_ = other.throw_except_ = true;
        other.was_init_ = is_init; // set to networking namespace's tracker of network is initialized (for windows)
    }
...
bool networking::network_structures::host::blocking() const {
    if (valid_socket(this->connect_socket_)) {
        if (networking::socket_is_connected(this->connect_socket_, this->throw_except_)) {
            return networking::socket_is_blocking(this->connect_socket_, this->throw_except_);
        }
    }
    return false;
}
...
networking::network_structures::host& networking::network_structures::host::blocking(const bool block) {
    if (valid_socket(this->connect_socket_)) {
        if (networking::socket_is_connected(this->connect_socket_, this->throw_except_)) {
            bool blocking = networking::socket_is_blocking(this->connect_socket_, this->throw_except_);
            if (block and not blocking) {
                // set the socket to blocking
                
                networking::set_blocking(this->connect_socket_, this->throw_except_);
            }

            else if (not block and blocking) {
                // set the socket to non-blocking
                
                networking::set_non_blocking(this->connect_socket_, this->throw_except_);
            }
        }
    }
    return *this;
...
// print_on_exceptions(const bool print)
...
networking::network_structures::host& networking::network_structures::host::throw_exception(const bool exception_) {
    this->throw_except_ = exception_;
    return *this;
}

// throw_exception() const
bool networking::network_structures::host::throw_exception() const {
    return this->throw_except_;
}
...
networking::network_structures::host::host(std::move(other)) {
    ...
    if (this != &other) {
        ...
        other.secure_was_init_ = true; // This way other server doesn't shut down secure if this server needs it.
    }
}
...
networking::network_structures::tcp_server& networking::network_structures::tcp_server::operator=(const networking::network_structures::tcp_server& other) {
    if (this != &other) {
        this->close_server();
        networking::network_structures::host::operator=(other);
...
networking::network_structures::tcp_server& networking::network_structures::tcp_server::operator=(networking::network_structures::tcp_server&& other) noexcept {
    if (this != &other) {
        ...
        // In question
        other.clients_.clear();
        other.secure_was_init_ = true; // This way the secure is not uninitialized if the current server needs it.
...
// disconnect_client(const client& client, const bool update_max, const bool disconnect_from_set)
...
// disconnect_client(const client_name& client_name, const bool update_max, const bool disconnect_from_set)
...
// disconnect_client(const std::string name, const std::string port, const bool update_max, const bool disconnect_from_set)
...
networking::network_structures::connected_host::client networking::network_structures::tcp_server::new_client(struct timeval timeout) {
    ...
    if (select(this->connect_socket_ + 1, &ready, 0, 0, &timeout) < 0) {
        message = "Failed to select for server's actively listening socket. Error " + std::to_string(socket_error) + " : " + std::string(get_socket_error_string(socket_error));
        if (this->throw_except_) {
            throw networking::exceptions::select_failure(message, true, __FILE__, __LINE__ - 1, __FUNCTION__);
        }
        std::cerr << message << std::endl;
        return the_answer;
    }
    ...
    if (FD_ISSET(this->connect_socket_, &ready)) {
        ...
        if (getnameinfo((struct sockaddr*) &the_answer.address_info, the_answer.address_size, address_buffer, buffer_size, port_buffer, buffer_size, NI_NUMERICHOST | NI_NUMERICSERV)) {
            ...
            if (getnameinfo((struct sockaddr*) &the_answer.address_info, the_answer.address_size, address_buffer, buffer_size, port_buffer, buffer_size, NI_NAMEREQD | AI_ALL)) {
                message = "Failed to retrieve client's name and or client's port number. All is necessary information. Error " + std::to_string(socket_error) + " : " + std::string(get_socket_error_string(socket_error));
                if (this->throw_except_) {
                    throw networking::exceptions::getnameinfo_failure(message, true, __FILE__, line_, __FUNCTION__);
                }
                std::cerr << message << std::endl;
                return the_answer;
            }
    ...
    if (not *this) {
        ...
        if (this->secure_ and not valid_context(this->context_)) {
            ...
            if (not this->create_context()) {
                line_ = __LINE__ - 1;
                ERR_error_string_n(ERR_get_error(), msg, count * buffer_size);
                message = "Failed to create server context_. Error \"" + std::string(msg) + "\"";
                if (this->throw_except_) {
...
networking::network_structures::tcp_client::tcp_client(const std::string host_address, const std::string port, const bool secure) :
    networking::network_structures::host::host(host_address, port, true, false) {
    this->secure_ = secure;
...
// Copy constructor
networking::network_structures::tcp_client::tcp_client(const tcp_client& other) :
...
// Move constructor
networking::network_structures::tcp_client::tcp_client(tcp_client&& other) noexcept :
    networking::network_structures::host::host(other) {
        this->secure_ = other.secure_;
...
networking::network_structures::tcp_client& networking::network_structures::tcp_client::operator=(const networking::network_structures::tcp_client& other) {
    if (this != &other) {
        networking::network_structures::host::operator=(other);
        // this->serve(false); // I don't think this is necessary
        this->secure_ = other.secure_;
        this->connect_time_ = other.connect_time_;
        this->secure_was_init_ = other.secure_was_init_;
        this->certificate_ = other.certificate_;
...
networking::network_structures::tcp_client& networking::network_structures::tcp_client::close_client() {
    if (*this) {
...
std::string networking::network_structures::tcp_client::get_subject_name(const std::string on_failure) const {
    std::string the_answer = on_failure;
    if (this->certificate_) {
        char* temp;
        temp = X509_NAME_oneline(X509_get_subject_name(this->certificate_), 0, 0);
        if (temp) {
            the_answer = std::string(temp);
            OPENSSL_free(temp);
        }
    }
    return the_answer;
...
std::string networking::network_structures::tcp_client::get_issuer_name(const std::string on_failure) const {
    std::string the_answer = on_failure;
    if (this->certificate_) {
        char* temp;
        temp = X509_NAME_oneline(X509_get_issuer_name(this->certificate_), 0, 0);
        if (temp) {
            the_answer = std::string(temp);
            OPENSSL_free(temp);
        }
